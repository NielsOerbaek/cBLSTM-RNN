...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov  8 15:21:03 2018
...
mask = Masking(mask_value ="<Mask>")(train_X)
...
if generate_model:
    ...
    model.add(LSTM(pp.max_sent_length, input_shape=(pp.max_sent_length, hidden_size), return_sequences=True)) ## TWO TIMES?
    model.add(TimeDistributed(Dense(pp.vocab_size, activation="softmax")))   ##TWO ACTIVATION FUNCTIONS? BOTH TANH AND SOFTMAX
    #model.add(Activation('softmax')(mask))
    ...
    # train LSTM
    model.fit(train_X, train_y, epochs=1, batch_size=10, verbose=1, callbacks=[checkpointer])

    model.save(model_name)
else:
    #model = load_model(model_name)
    model = h5py.File("mytestfile.hdf5", "r")
