...
positive_LM_filename = model_folder_path + "cBLSTM-pos-LM-10.hdf5"
negative_LM_filename = model_folder_path + "cBLSTM-neg-LM-10.hdf5"
binary_classifier_filename = model_folder_path + "BLSTM-BC-05.hdf5"
num_samples = 0
...
for i in range(len(train_X[:train_limit])):
    ...
    print("Calibrating:", i, "of", train_limit)
...
classifier_model.add(Dense(5))
classifier_model.add(Dense(1, activation="sigmoid"))
classifier_model.compile(optimizer='sgd', loss='mse', metrics=['accuracy'])

print("Making classifier model training data")
classifier_X = np.zeros(shape=(train_limit, 2))
classifier_y = np.zeros(shape=(train_limit))
...
for i in range(len(train_X[:train_limit])):
    ...
    LM_classification = classify_review_by_lm(train_X[i], pos_predictions, neg_predictions)
    BC_classification = classify_review_by_bc(train_X[i], bc_predictions)
    classifier_X[i] = [LM_classification, BC_classification]
    classifier_y[i] = train_y[i]
    print(str(round(i / train_limit * 100, 2))+"%", end="-", flush=True)
    if i % 20 == 0:
        print()
...
classifier_model.fit(classifier_X, classifier_y, epochs=20)
...
for i in range(samples):
    ...
    classification = round(float(classifier_model.predict([[[LM_classification, BC_classification]]])[0]))
